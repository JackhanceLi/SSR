...
def training(file_name, chan_num):
    ...
    stable_stp = 750  # 固定的起始点（以防有些epoch检测不到运动起始时间）
    ...
    for epo in range(epoch_data.shape[0]):
        for fnum in range(f_num):
            for chan in range(chan_num):
                ...
                f_rms_tr[epo, chan+fnum*chan_num] = np.sqrt(np.sum(epoch_data[epo, chan,
    ...
    # 分类器训练
    svc.fit(f_rms_tr, labels)
    clf.append(svc)  # 保存svm模型

    joblib.dump(clf, 'online_'+str(chan_num)+'chans_clf.m')
...
if __name__ == '__main__':
    ...
    file_name_ = './data/20221122T173103.txt'
    chan_num_ = 5
